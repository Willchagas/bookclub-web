...
import { Button, CategoryList, NavBar, Text } from 'components'
...
export const BookDetailScreen = () => {
  ...
  const addFavoriteMutation = useMutation((data) => addBookToFavorites(data), {
    onError: (error) => {
      toast({
        title: 'Livro não adicionado aos favoritos!',
        description:
      ...
      >
        ...
        <Flex w="70%" mx="48px" flexDir="column">
          ...
          <Text mt="6px" fontSize="16px" color="brand.greyDark">
            Autor: {data?.data?.book?.author?.name}
          </Text>

          <Text.ScreenTitle mt="18px">Informações</Text.ScreenTitle>
          ...
          <Text.ScreenTitle mt="18px">Sinopse</Text.ScreenTitle>
          <Text mt="12px" fontSize="12px" color="brand.greyDark">
